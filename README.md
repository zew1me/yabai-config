...
- ↔️ **Side snap shortcuts** - Snap focused window to left/right side
...
- Default keybindings:
  ...
  - `⌘⌃←` for left snap (focused window)
  - `⌘⌃→` for right snap (focused window)
...
# Side snap defaults
KEYBINDING_SNAP_LEFT="cmd + ctrl - left"
KEYBINDING_SNAP_RIGHT="cmd + ctrl - right"
```
...
# Snap focused window directly
./snap_current_window_side.sh left
./snap_current_window_side.sh right
```
...
The `snap_current_window_side.sh` script:
...
5. **If snap hotkeys conflict with another app:**
...
├── snap_current_window_side.sh  # Snap focused window to left/right side
